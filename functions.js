...
function add_or_update_switch(device, device_no){
	...
	if (currentActionDiv.length === 0) {
		var deviceDiv = createElement("div", "rctngl1 ");
...
function createActionLink(device, online, state, type){
	var onString = type === "scene" ? "Start" : "Connected";
	if (online === false) {
		return '<a href="#" class="borderShadow ui-btn ui-disabled ui-btn-inline ui-icon-power ui-btn-icon-left">Offline</a>';
	} else if (state === false) {
		return '<a href="#" class="borderShadow ui-btn ui-btn-b ui-btn-inline ui-icon-power ui-btn-icon-left" onclick="toggle('+device+');">Disconnected</a>';
	} else {
